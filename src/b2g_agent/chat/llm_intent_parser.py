...
class LLMIntentParser:
    ...
    def __init__(self, model: str | None = None, client: Any | None = None, debug: bool = False) -> None:
        _load_env()
        self.model = model or os.getenv("B2G_CHAT_MODEL", "gpt-4.1-mini")
    ...
    def parse(self, text: str) -> list[Intent]:
        ...
        if self.debug:
            print(f"[b2g-chat-debug] Raw LLM response: {raw}")
        try:
            request = validate_chat_action_request(raw)
        except LLMIntentValidationError as exc:
            if self.debug:
                print(f"[b2g-chat-debug] LLM validation error: {exc}")
            raise
...
def _system_prompt() -> str:
    return """
You are the B2G-Agent Building-to-Grid intent parser.
